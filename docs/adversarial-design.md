...
3. **Fact-utilization is a lower bound.** `measure_fact_utilization` uses a 30-character case-insensitive substring match. LLMs paraphrase, so a false negative just means we under-count. Two further biases (#191): `injected` counts the sibling tier's first-sentence summaries alongside full-text core facts, and those are the claims a component is least likely to echo, so the denominator is inflated; and the measurement only runs for components that reach the distill phase, which is after verification, review, and security all pass - a component that FAILED review had facts injected and may have used them, but is never sampled. Read the population as "successful components only".
