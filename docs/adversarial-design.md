...
| **Human checkpoint (E6)** | `kstrl/factory.py` | (interactive) | Pre-merge | Optional. When `FactoryConfig.pause_before_pr_merge=True` and UI is interactive, prompts a human to approve or reject before PR creation. Off by default. Honoured only by PR-creating commands (`ks factory`, and `ks serve` via its factory invocations); `ks run` creates no PRs, so the gate does not apply there and a startup notice says so when the flag is set (#207). |
