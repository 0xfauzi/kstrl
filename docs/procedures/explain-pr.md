...
already carries. It names the file and lets the reader run the command. One
fact, one definition.
...
### 2. Fix the names the lesson will use

Every module, class or function the lesson names must resolve in the tree at
the head of the range. Check each one with
`git show origin/<headRefName>:<path>` before writing. A name that does not
resolve is a claim the lesson cannot make.
...
**Class A, pictures of the whole system.** There is no system-map generator in
this repository any more; #368 removed the last of it. A lesson does not
hand-draw one either, because a hand-drawn map is a claim about the tree that
nothing checks. Point at [ARCHITECTURE.md](../../ARCHITECTURE.md) and spend
the page on the mechanism the change introduces.
...
except for a supersession banner.
...
names what it reverses. The register's `parts:` are only as good as the reading
that produced them. The checker counts sentences, not teaching. The fresh reader is fresh
