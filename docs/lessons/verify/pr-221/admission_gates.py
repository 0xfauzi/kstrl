...
def admit(
    ...
    if not ledger_readable:
        return {"gate": "spend ledger", "kind": "halt",
    ...
    if poison_streak >= MAX_CONSECUTIVE_POISON:
        return {"gate": "poison breaker", "kind": "pause",
...
ORDER = ["spend ledger", "pause marker", "poison breaker", "cost coverage",
         "daily budget", "open-PR bound", "inbox cap", "factory lock", "claim"]
...
def main() -> None:
    ...
    print("claim 1: only the three ledger gates ever pause the queue ->",
          "holds" if pauses == {"poison breaker", "cost coverage", "daily budget"} else "fails")
    print("claim 2: the open-PR bound only ever waits, never pauses ->",
          "holds" if all(r["kind"] == "wait" for r in rows if r["gate"] == "open-PR bound")
          else "fails")
    print("claim 3: before R10.7 the open-PR inputs never decide anything ->",
          "holds" if not any(r["gate"] == "open-PR bound" for r in rows if not r["after_r10_7"])
          else "fails")
    print("claim 4: a failed gh count refuses; it is never read as zero ->",
          "holds" if all(r["gate"] == "open-PR bound" for r in rows
                         if r["after_r10_7"] and not r["gh_ok"] and r["max_open_prs"] > 0
                         and r["create_prs"] and not r["paused"] and r["ledger_readable"]
                         and r["poison_streak"] < 3
                         and not (r["budget_on"] and not r["allow_uncovered"] and not r["coverage_seen"])
                         and not (r["budget_on"] and r["budget_reached"])) else "fails")
    print("claim 5: bound 0 or create_prs=false never consults gh ->",
          "holds" if not any(r["gate"] == "open-PR bound" for r in rows
                             if r["max_open_prs"] == 0 or not r["create_prs"]) else "fails")
    print("claim 6: the default bound (1) refuses as soon as one kstrl PR is open ->",
          "holds" if admit(True, True, False, 0, False, False, True, False, 1, 1, True, True,
                           False, False, True)["gate"] == "open-PR bound"
          and admit(True, True, False, 0, False, False, True, False, 1, 0, True, True,
                    False, False, True)["kind"] == "claim" else "fails")
    print("claim 7: an unreadable ledger halts before any other gate is read ->",
          "holds" if all(r["kind"] == "halt" for r in rows if not r["ledger_readable"])
          else "fails")
    print("claim 8: with everything nominal and an item ready, the cycle claims ->",
          "holds" if admit(True, True, False, 0, False, False, True, False, 1, 0, True, True,
                           False, False, True)["kind"] == "claim" else "fails")
    print("claim 9: the budget gates are inert while daily_budget_usd is 0 ->",
          "holds" if not any(r["gate"] in ("cost coverage", "daily budget") for r in rows
                             if not r["budget_on"]) else "fails")
    print()
    print("observation: the open-PR bound is evaluated before the inbox cap and the "
