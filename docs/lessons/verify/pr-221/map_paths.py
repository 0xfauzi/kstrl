...
WALK: list[dict[str, object]] = [
    {
        ...
        "nodes": ["GitHubIssues", "GitHubIntake", "WorkQueue"],
    ...
    },
    {
        ...
        "nodes": ["Scheduler", "Manifest", "Worktrees"],
        "measures": [],
    ...
    },
    {
        ...
        "nodes": ["Findings", "Pipeline", "RetryContext", "AutonomyLadder"],
        "measures": ["Findings"],
        "edges": [
            ["Findings", "Pipeline"],
    ...
    },
    {
        ...
        "nodes": ["PullRequests", "ContractTester", "Scheduler"],
        "measures": ["ContractTester"],
        "edges": [["PullRequests", "ContractTester"], ["ContractTester", "Scheduler"]],
    },
    {"title": "release (planned, R8.7)", "nodes": ["ReleaseStage"], "measures": [], "edges": []},
    {
        "title": "runtime signals (planned, R8.8)",
        "nodes": ["RuntimeSignals"],
        "measures": ["RuntimeSignals"],
        "edges": [],
    },
    {
    ...
    },
    {
        ...
        "nodes": ["Scheduler", "EvolutionJournal", "Proposals", "Replay", "Playbook"],
        "measures": ["Replay"],
        "edges": [
...
LOOPS: list[dict[str, object]] = [
    {
        ...
        "closed": "open",
        "actuator": "EngineerLoop",
        "sensor": "none: the prompt is re-sent unchanged each iteration",
    ...
    },
    {
        ...
        "sensor": "MechanicalVerifier, Reviewer, SecurityReviewer",
        "setpoint": "the component PRD",
        "nodes": [
            "Pipeline",
            "RetryContext",
            "MechanicalVerifier",
    ...
    },
    {
        ...
        "closed": "closed",
        "actuator": "Scheduler (schedule, merge, reset breaker)",
        "sensor": "ContractTester",
    ...
    },
    {
        ...
        "rate": "hours",
        "closed": "closed",
        "actuator": "ServeDaemon (queue admission)",
        "sensor": "SpendLedger, Inbox, FlowControl",
    ...
    },
    {
        ...
        "rate": "weeks",
        "closed": "open",
        "actuator": "playbook and prompt edits",
        "sensor": "attribution, Calibration",
    ...
    },
    {
        ...
        "actuator": "ReleaseStage (R8.7)",
        "sensor": "RuntimeSignals (R8.8)",
        "setpoint": "the service's SLO",
        "nodes": ["ReleaseStage", "RuntimeSignals"],
        "regions": ["ship", "learn"],
    },
    {
        "id": "observe",
        ...
        "sensor": "none: it is the view every loop is read through",
        "setpoint": "none",
...
def main() -> None:
    ...
    print(
        f"walk: {len(WALK)} steps, {len(walked)} distinct components, "
        f"{sum(len(s['edges']) for s in WALK)} edges"
    )  # type: ignore[arg-type]
    print(f"loops: {len(LOOPS)}, covering {len(looped)} of {len(ids)} components")
    print("components in no loop:", sorted(ids - looped))
    print(
        "claim: every id and edge the two widgets light exists in logical_model ->",
        "holds" if not p else "fails",
    )
    for line in p:
        print("  ", line)
    print(
        "claim: every proposed change maps to exactly one loop ->",
        "holds"
        if all(sum(1 for lo in LOOPS if lo["id"] == ch["loop"]) == 1 for ch in CHANGES)
        else "fails",
    )
