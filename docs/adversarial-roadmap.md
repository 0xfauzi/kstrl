...
- [x] H3 - Adopted 2026-05-27. Iterated three times the same day. Every adversarial prompt (`DECOMPOSE_PROMPT`, `REVIEWER_PROMPT`, `SECURITY_PROMPT`, `DISTILL_PROMPT`, `DEFAULT_PROMPT` for engineer role, `DEFAULT_PRD_PROMPT` for PRD creation) declares a `*_PROMPT_VERSION` semver constant. `tests/test_prompt_versions.py::_EXPECTED_SNAPSHOTS` records `(hash, version)` jointly: both must move together. AST auto-discovery test (`test_no_unenrolled_prompt_constants`) uses `ast.walk` to catch every prompt-shaped constant, including typed assignments (`X: str = "..."`) and declarations nested inside functions or class bodies. The engineer prompt template AND the PRD-creation prompt are in scope; the latter was added in the post-retrospective cleanup because it shapes the PRD that every downstream phase consumes. **What the walk keys on is the target NAME plus a literal value: it flags an assignment at any nesting depth whose name ends in `_PROMPT` and whose right-hand side is a plain string or f-string.** So it is blind to instruction text that is never bound to such a name - returned straight out of a function, or bound to a local called something else - and blind to a body assembled by concatenation or `str.join` whatever the name. #299 found both blindnesses live in `git.py`: `repo_change_source` returned its body with no binding at all, and `pasted_change_source` bound its body to `block`, and either could be reworded with the whole suite green. **The rule: instruction text reaching a role goes in an enrolled module-level `*_PROMPT` constant carrying `{placeholder}` sites, and the function that needs it interpolates the run-time values back in.** That is an obligation on the author, and the walk cannot detect its absence - it can only catch the body you did name. Two corollaries close the failures that ARE mechanical: the per-prompt hash check is parametrized over the enrolled set rather than hand-written per prompt, so a prompt cannot be enrolled yet unguarded the way `VERIFY_COMMANDS_PROMPT` was from #261 to #299; and `test_change_source_functions_render_the_enrolled_body` asserts each change-source function renders exactly its enrolled body, so a constant cannot rot into an orphan that matches its snapshot for ever while the role receives different words.
