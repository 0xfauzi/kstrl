...
- [x] E3 - Typed `Finding` dataclass at `ralph_py/findings.py`. `Component.findings: list[Finding]` is the new source of truth, `Component.review_findings` (string) is a derived fallback kept for legacy manifests. `ReviewResult.as_findings()` converts criteria failures + concerns; `SecurityResult.as_findings()` adds OWASP/CWE taxonomy. Hardening after retrospective: (a) `pr.py` now renders from typed findings via `render_findings_markdown`; (b) evolution journal serializes `findings` + a `findings_summary` aggregator (E3-consume); (c) infrastructure_error promoted into a synthetic Finding so `len(findings)==0` safely means "ran cleanly" (E3-infra); (d) `phase:`, `category:`, `owasp:`, `cwe:` tags populated on every Finding (E3-tags).
...
- [x] H3 - Adopted 2026-05-27. Hardened later same day after the retrospective surfaced the version-without-bump bypass. Every adversarial prompt (`DECOMPOSE_PROMPT`, `REVIEWER_PROMPT`, `SECURITY_PROMPT`, `DISTILL_PROMPT`, `DEFAULT_PROMPT` for the engineer role) now declares a `*_PROMPT_VERSION` semver constant. `tests/test_prompt_versions.py::_EXPECTED_SNAPSHOTS` records `(hash, version)` jointly: both must move together. AST auto-discovery test (`test_no_unenrolled_prompt_constants`) fails if any new `*_PROMPT` in `ralph_py/` is added without enrollment. The engineer prompt template is now in scope.
