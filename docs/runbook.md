...
- `prd_stories`: the agent never set `passes: true` on its assigned story. Either the iteration ran out, or the agent didn't understand the PRD. It also fails with "The PRD is not the one this run started with" when the component rewrote its own PRD in a way no engineer may: a story's criteria, title, priority or id, the approved fixtures, or `branchName`. Only `passes` and `notes` are the engineer's to write. Restore the file from the pre-run copy in the main checkout; do not adjust what the component is measured against. Editing `allowedPaths` is NOT this failure and never has any effect: scope is resolved once, before the first engineer call, and neither guard re-reads the PRD for it. Editing `specIssues` is not this failure either: it is the architect's spec audit, routed here so the engineer reads it, and no gate judges the component against it. The full audit is in `scripts/kstrl/spec-issues.json` if you need the copy the component cannot reach.
