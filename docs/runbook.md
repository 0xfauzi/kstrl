...
- `diff_scope`: the agent wrote files outside `ALLOWED_PATHS`. Tighten the allowlist or relax it as appropriate. Do NOT widen it to cover kstrl's own files: on `ks factory`, `ks run` and `ks understand`, the component's PRD, its progress log and `scripts/kstrl/codebase_map.md` are carved out automatically per component and reported separately in the failure as `plus harness artifacts:`. Widening to the bare `scripts/kstrl/` prefix to reach them exposes the manifest and every sibling component's PRD. `ks feature` carves out its own `.kstrl/logs/` run directory the same way. The allowlist it enforces is the one this run resolved before its first engineer call, from the component's `allowedPaths` or, when the PRD carries none, from the run-wide `--allowed-paths`; the `component_scope_resolved` event in the run's `events.jsonl` records which, per component. Editing the PRD mid-run does not move it. A scope that could not be READ at all is a different check, `scope_source`, below.
- `scope_source`: neither the component's pre-run PRD nor `--allowed-paths` gave this run a scope it could trust, so Phase 1 fails closed and no diff can be judged. Its own check since #294, and ungated: `[verify] check_diff_scope = false` switches off the diff COMPARISON, not the report that there was nothing to compare against. Nothing the engineer writes can clear it - the scope is read from the pre-run checkout, outside every worktree, and is fixed for the life of the run - so this is an operator fix: restore the component PRD, or set `--allowed-paths`, and start a new run. On `ks factory` you should never see it, because `_preflight_component_scope` refuses an unresolved component before the first engineer call; seeing it means Phase 1 was reached by some other route.
