...
- `diff_scope`: the agent wrote files outside `ALLOWED_PATHS`. Tighten the allowlist or relax it as appropriate. Do NOT widen it to cover kstrl's own files: the component's PRD, its progress log and `scripts/kstrl/codebase_map.md` are carved out automatically per component and reported separately in the failure as `plus harness artifacts:`. Widening to the bare `scripts/kstrl/` prefix to reach them exposes the manifest and every sibling component's PRD.
