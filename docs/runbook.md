...
**What it costs**: one test + typecheck + lint run for the baseline, plus one per engineer loop, so `2 + repair_max_runs` at worst. Measured on the kstrl repo itself: 246s per report, essentially all test suite, against 317-348s for the engineer loop each one follows. On a project with a fast suite it is seconds.
