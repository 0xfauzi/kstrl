...
- `scope_unreadable`: the component's pre-run PRD could not be read or parsed, so this run has no scope it can trust and Phase 1 fails closed without judging any diff. Its own check since #294, and ungated: `[verify] check_diff_scope = false` switches off the diff COMPARISON, not the report that there was nothing to compare against. **Restore the component's PRD in the main checkout and start a new run. Setting `--allowed-paths` does NOT work here**, even though it is the fallback when a PRD simply carries no `allowedPaths`: scope resolution returns `unresolved` before it reaches the flag, on the argument that a scope nobody could read is not a scope that does not exist, so a re-run with the flag set fails identically. Nothing the engineer writes can clear it either, since the file is outside every worktree and the snapshot is fixed for the life of the run - so the component is failed immediately rather than retried (#294); you will not see three attempts burn on it. Usually `_preflight_component_scope` catches this before the first engineer call and refuses the whole run, so reaching Phase 1 means the component was not PENDING when the preflight ran: the contract breaker resets a COMPLETED component to PENDING mid-run, and a component whose PRD went missing between runs takes exactly that route.
