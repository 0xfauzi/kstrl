...
Provenance is key PRESENCE, never a value comparison: `KSTRL_FACTORY_PAUSE_BEFORE_PR_MERGE=0` is an explicit `false`, not an absent setting. The `[factory] pause_before_pr_merge` toml key is read strictly for the same reason `[autonomy] enabled` is: `pause_before_pr_merge = "false"` is a string, every non-empty string reads as true, and a coerced string would manufacture an explicit request nobody wrote. A quoted value is refused at command entry as a configuration problem rather than loaded. The env var is unchanged and stays lenient (`1`/`true`/`yes` are true, anything else false), because an env var is a string by construction.
