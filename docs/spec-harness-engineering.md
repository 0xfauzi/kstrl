# Spec: Harness Engineering for kstrl
...
kstrl's factory mode has strong feedback sensors (3-phase verification: mechanical, review, contract) but weak feedforward controls. The agent runs, then we check. When it fails, we inject failure context and retry - but we never improve the harness itself between runs. This creates two problems:
...
2. **No learning across runs**: kstrl retries with context but never accumulates reusable knowledge. The same architectural mistakes recur across components and across factory runs. AutoResearchClaw's "evolution directory" and autoresearch-agents' `results.tsv` both demonstrate that structured experiment journals dramatically improve convergence.

Additionally, factory mode is currently a secondary command (`ks factory`) despite being the more capable execution path. Single-component `ks run` should be a degenerate case of factory, not the default.
...
- **Feedforward + feedback in balance**: "You get either an agent that keeps repeating the same mistakes (feedback-only) or an agent that encodes rules but never finds out whether they worked (feedforward-only)." kstrl today is feedback-dominant.
...
- **Leverage existing tools, don't reinvent them**: Ruff, mypy, pytest, eslint already catch problems. kstrl's job is to make their output actionable for agents (structured parsing, source context, convention awareness) - not to build a parallel rule engine.

- **The steering loop improves the harness, not just the output**: When issues recur, the controls themselves should be updated. This is the meta-loop that transforms kstrl from a retry system into an adaptive one.
...
**What**: `ks run` becomes a thin wrapper that creates a single-component manifest and delegates to factory execution. The `factory` command becomes the primary code path. `ks run` is preserved as a convenience alias.
...
```
# Before: ks run uses loop.py directly, no verification
ks run 25

# After: ks run creates a 1-component manifest and runs factory
...
- `ks run` synthesizes a `Manifest` with one component from the existing PRD
...
- `ks run --no-verify` preserves the old behavior (raw loop, no verification)
- `ks run --legacy` invokes `run_loop()` directly as an escape hatch during migration

**Files to modify**:
- `kstrl/cli.py`: Rewrite `run` command to build manifest and call `run_factory`
- `kstrl/factory.py`: Support `use_worktrees=False` with `max_parallel=1` (already works, just needs testing)
- `kstrl/manifest.py`: Add `Manifest.from_prd(prd_path, branch)` class method for single-component manifests

**Risks**:
- Users relying on `ks run` completing faster because it skips verification. Mitigation: `--no-verify` flag.
...
**What**: Before each component runs, kstrl collects codebase structure and injects it into the prompt:
...
- Existing `codebase_map.md` if available (from `ks understand`)
...
#### 3.2.2 Why not custom kstrl rules?

The article advocates for "custom linter messages that include instructions for self-correction." This is a valid insight, but we don't need a kstrl-specific rule engine to achieve it. Ruff, mypy, eslint, and other existing linters already catch the problems. What's missing is making their output actionable for agents.

kstrl addresses this through two mechanisms that already exist in this spec:
...
2. **LLM-optimized sensor output (3.3)**: When ruff flags S608, kstrl parses it into a structured message with the file path, source context, and the relevant convention from feedforward. The agent gets project-specific guidance without kstrl reinventing linting.

If this proves insufficient after measuring iteration counts, a lightweight annotation mapping (ruff rule code -> project-specific fix hint) could be added to `kstrl.toml` later. But start without it.

#### 3.2.3 Bootstrap scaffolding

**What**: Before the agent starts a component, kstrl can optionally run a scaffolding step that creates file stubs, interface definitions, or test harnesses. This constrains the agent's solution space structurally.
...
{
  "id": "auth-service",
  "scaffold": "scripts/kstrl/scaffolds/auth-service.sh",
  "userStories": [...]
}
```
...
**Files to modify**:
- `kstrl/verify.py`: Each `check_*` function returns structured `CheckResult` with parsed details instead of raw output lines
- `kstrl/context.py`: `format_for_prompt` uses structured data to generate targeted fix suggestions
- New: `kstrl/parsers.py` for pytest, mypy, ruff output parsing
...
**What**: After each factory run, kstrl analyzes recurring failure patterns and proposes harness improvements. This is the meta-loop: the harness improves itself, not just the code.
...
**What**: Address the article's "elephant in the room" - functional correctness verification. kstrl currently relies on agent-generated tests, which the article warns "puts a lot of faith into AI-generated tests, that's not good enough yet."
...
**New file**: `kstrl/fixtures.py`
**Modified file**: `kstrl/verify.py` (add `check_fixtures`)
**Modified file**: `kstrl/prd.py` (add optional `fixtures` field)

#### 3.5.2 Snapshot regression

**What**: After a component passes all verification phases, kstrl snapshots key outputs (API responses, rendered HTML, CLI output). On subsequent runs or retries, regression is detected by comparing against snapshots.

**How**: Snapshots stored in `.kstrl/snapshots/<component_id>/`. During Phase 1, if snapshots exist from a previous successful run, they're compared against current output.
...
All new configuration lives in `kstrl.toml` under new sections. No new config files required.
...
```
ks run [N]                  # now delegates to factory (single-component)
...
| Factory-as-default breaks existing `ks run` scripts | User CI pipelines break | `--legacy` flag for exact old behavior; `--no-verify` for lightweight factory |
...
- **Real-time monitoring / alerting**: No `kstrl watch` or continuous background monitoring. The evolution journal runs post-hoc, not in real-time. Background monitoring can be a follow-up.
...
- **Custom kstrl lint rules**: Existing linters (ruff, mypy, eslint) already catch problems. kstrl's value is making their output actionable, not reimplementing linting. See section 3.2.2 for rationale.
