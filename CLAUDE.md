...
- **A parser's error taxonomy belongs to the parser.** A `tomllib` reader catches `Exception`, never an enumeration of what it is believed to raise. `tomllib.load` decodes the stream before it lexes and parses by recursive descent, so it raises `TOMLDecodeError`, `UnicodeDecodeError`, plain `ValueError` (CPython's 4300-digit integer-string limit) AND `RecursionError` (~496 nested arrays; a `RuntimeError`, not a `ValueError`). #318 enumerated subclasses three times and was wrong three times - each escape took 13 of 16 CLI commands down with a raw traceback, because `config_preflight` reads the document for every non-exempt command. Round 2 is the instructive one: it stated this rule correctly and then wrote `ValueError` as the ceiling in its docstring, its AST guard AND this line, so a future author could satisfy all three and still ship the hole. Do not name a ceiling narrower than "an exception, out of this call"; `Exception` is the honest one because everything about the DOCUMENT derives from it while `KeyboardInterrupt`/`SystemExit` are about the process. Re-raise `OSError` explicitly above the catch-all rather than relying on it not being a `ValueError`, keep `open()` outside the guarded block so an unopenable path is not reported as a parse failure, and report individually only the causes you can actually name, broad clause LAST. `kstrl/config.py::load_toml_document` is the worked example. `tests/test_toml_readers.py` AST-walks `kstrl/` and fails on a `tomllib` parse whose handler does not name `Exception` or wider, resolving `import tomllib as _tl`, `from tomllib import load` and module rebinds; it does not see a parse reached through a name it cannot resolve, and says so.
