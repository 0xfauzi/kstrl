...
- A `tomllib` reader catches `ValueError`, never an enumeration of its subclasses. `tomllib.load` decodes the stream itself before it lexes, so it raises `TOMLDecodeError`, `UnicodeDecodeError` AND plain `ValueError` (CPython's 4300-digit integer-string limit), and the taxonomy is `tomllib`'s to extend. #318 enumerated the subclasses, shipped, was found still broken, enumerated one more, and was found still broken again - each time taking 13 of 16 CLI commands down with a raw traceback, because `config_preflight` reads the document for every non-exempt command. Catch the family and report the members you can name individually, in that order, with the broad clause LAST: `kstrl/config.py::load_toml_document` is the worked example. `tests/test_toml_readers.py` AST-walks `kstrl/` and fails on a `tomllib.load` call whose handler does not name `ValueError` or wider.
