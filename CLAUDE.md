...
- **Calibration is the truth signal.** When changing ANY enrolled prompt (the eleven in `tests/test_prompt_versions.py::_PROMPTS`: `DECOMPOSE_PROMPT`, `ARCHITECT_REPO_SOURCE_PROMPT`, `ARCHITECT_NO_REPO_SOURCE_PROMPT`, `REVIEWER_PROMPT`, `SECURITY_PROMPT`, `DISTILL_PROMPT`, `VERIFY_COMMANDS_PROMPT`, `REPO_CHANGE_SOURCE_PROMPT`, `PASTED_CHANGE_SOURCE_PROMPT`, `DEFAULT_PROMPT` for the engineer role, and `DECISIONS_CONTEXT_PROMPT` since #260, plus the 53 builder fragments enrolled by #303 via `tests/helpers/builder_prompts.py`), re-run the calibration suite and compare detection rates against the saved baseline. A prompt edit without a calibration check is treated as untested (H2). Two exceptions, stated rather than left implicit. `DECISIONS_CONTEXT_PROMPT` is engineer-facing CONTEXT, and the calibration suite scores the architect, reviewer, security and distiller roles against planted-bug fixtures, so it is enrolled for H3 and has no calibration fixture to re-run. The 53 #303 builder fragments are the same position: they are engineer-facing (the engineer retry path, mostly) and the calibration suite scores architect, reviewer and security only, so they carry the H3 obligation and no H2 obligation the suite can discharge. Enrolling a prompt that no fixture scores does not create an H2 obligation it cannot discharge; it does create the H3 one. The `ARCHITECT_REPO_SOURCE_PROMPT` / `ARCHITECT_NO_REPO_SOURCE_PROMPT` pair (#199) carries the H3 obligation, discharged in the PR that enrolled them, but their H2 obligation cannot be discharged until #401 supplies an architect calibration fixture with a repository to read.
...
- **A static guard fails in the skip direction, so design it to fail red instead.** This is the most-repeated defect in the repo: eleven logged instances of *a guard that passes the mutation it exists to catch*, every single miss in the skip direction, the guard going blind rather than red. The count is sourced: #324 records ten, and `tests/test_timeout_enforcement.py` names its own as instance eleven in a comment on `CHILD_WAIT_SCOPE`. Four rules came out of them. (1) Prefer **closed by construction** over a **ledger of give-ups**: inventory every place the resource is OBTAINED and count what you saw, so a new shape shows up as an unexplained census delta; a ledger of the places the walk gave up is closed only over the shapes someone already enumerated. `EXPECTED_JOURNAL_PATH_SITES` in `tests/test_journal_one_writer.py` is the worked example, and `tests/test_event_names_have_one_home.py` is the current best: its outer layer enumerates no node types at all and counts every expression whose folded value is an enrolled name. (2) **`assert hits(...) == []` is not a control.** It passes when the walk is correctly narrow AND when the walk has been switched off, which is the same shape as the defect. Express a genuine disclosed miss as an explicit blind-spot record plus a strict xfail, so a later widening XPASSes and fails loudly instead of silently agreeing. (3) **Know which direction your guard is wrong in.** A guard that FLAGS may over-match: widening costs a false positive someone will read. A guard that CLEARS must be narrow, because over-matching converts a resolution into a clearing and deletes the mechanism; `tests/helpers/astwalk/__init__.py` records the split as thirteen of the sixteen migrated guards flagging and three clearing, and PR #342 finding B3 measured one of the three clearing an unguarded `build_config_report` call reachable on the Textual event loop. If a clearing guard cannot PROVE a site is compliant, it must flag. (4) **A guard you did not mutate is a guard you did not test**, per layer, not per file.
- **Reading a pin is not running a guard, and a guard must not depend on the code path whose absence it is detecting.** Two ways a control stops being true with nothing failing. The first, from PR #341: a lane reported a census pin identical at two revisions as evidence that a large refactor moved nothing, when EXECUTING the guard at those revisions was red, because the refactor had split a file without updating the pin, so the census counted the two halves separately while the pin still named one file. The identical dict was a symptom of the stale pin, not evidence of stability. Re-derive a census by running it; never by diffing the literal. The second: a runaway-detector fuse sat inside a stubbed `pause` that the very defect it detects stops calling, so it **hung for 26 minutes instead of failing**. Every earlier instance of the class went blind, which at least returns; this one went slow, which is worse. Put the fuse somewhere the defect cannot switch off, such as real time rather than the mocked clock.

- **Three ways a measurement lies rather than the code being wrong.** All three cost real rework in the PR #341 sweep, and none of them fails: each returns a plausible number. (1) **A mutation that removes a synchronization point fails as a HANG, not as a red test.** Deleting a wait does not make the assertion fail, it makes the assertion never run, so an unbounded run cannot tell "caught" from "still going". Bound every mutation run per unit (#341 used 420s per file, 180s per plant) and record `hung` as an outcome distinct from `caught` and `still green`; three instances in one PR, one of them 26 minutes at 0.3% CPU. (2) **A flag that suppresses an output turns a count of that output into a zero, and a zero looks like a measurement.** `--tb=no` strips the exception off the summary line, so counting `NoMatches` under it reported 0 where the same tree gives 39 under `--tb=line`. Keep raw stdout so a classification can be redone without re-running. (3) **`subprocess.run(timeout=)` kills only what it spawned.** Shelling out through `uv` means the timeout kills `uv` while the `pytest` underneath survives, and the orphans then compete for CPU with every later measurement. Use `start_new_session=True` and `os.killpg`. A process outliving the thing meant to bound it happened three times in one session, which is #326's own defect class occurring in the measurement tooling rather than in the factory.
- **A mutation that silently fails to plant is the same false all-clear as one aimed at the wrong file.** Both were hit in PR #344. A refactor moves a mutation's anchor, the edit applies to nothing, the suite is green, and the run is scored as caught. Report COULD-NOT-PLANT as its own outcome and retarget, never fold it into a pass.
...
- Shared AST-walking machinery for static guards lives in `tests/helpers/astwalk/` (#324), not hand-rolled per guard. Measure the fit before adopting it rather than assuming it: a lane that costed the migration for PR #341 found most of its helpers had no astwalk equivalent, several needing a parent map astwalk does not keep, being strictly top-down (`grep -rn parent tests/helpers/astwalk/` finds only `Path.parent`). `Bindings.attributes` marks an origin it inferred as `guessed`, and a `guessed` origin must not be enough to CLEAR.
...
- **One defect class, one PR.** Fix every instance plus the guard that catches instance N+1, rather than one site per PR with sibling issues filed behind it. The habit is visible in the labels: `gh issue list --state all --label absorbed` and `--label class-sweep` return ten issues filed on a single day, six of them siblings a class PR later absorbed. Fixing one site at a time is what produced them. Work that genuinely will not fit goes in the PR body as a measured handoff with the numbers attached, not as a new issue.
- **Merging into a moving main is where the defects are.** Several defects in one batch survived a textually clean merge and were caught only by running the full suite on the merged tree. Two are recorded: PR #342 merging #340 found `SPAWN_FUNCS` left with zero readers and both names ungated, with no conflict on either constant and both branches independently green; PR #341 merging #342 found a file at 750 lines on one side and 800 on the other merging to 827 against an 800-line ratchet, a failure present in neither branch and attached to no hunk. So merge current `origin/main` and re-run the full suite before pushing, and **re-run every static guard's site census before and after the merge and diff the two**, because a code move in another file changes what a walk sees without touching yours.
