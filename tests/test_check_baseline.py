"""R10.6 (#227): the baseline's arithmetic, and how a comparison renders.

No CLI here. These drive :mod:`kstrl.baseline` and
:mod:`kstrl.baseline_report` directly, so the bucket rules are pinned
independently of how ``ks check`` wires them up. Two siblings hold the rest:
``tests/test_check_baseline_document.py`` the artifact on disk and every way
reading it fails, and ``tests/test_check_baseline_cli.py`` the command surface.
...
BASELINE_DOC = Path(__file__).resolve().parents[1] / "docs" / "baseline.md"
...
def test_compare_no_regression() -> None:
    ...
    comparison = baseline.compare(base, _baseline({"linter:E501": 3}))
...
def test_compare_detects_increased_count() -> None:
    comparison = baseline.compare(_baseline({"linter:E501": 1}), _baseline({"linter:E501": 2}))
...
def test_compare_reports_fixed_without_regression() -> None:
    ...
    this one.
    """
    comparison = baseline.compare(_baseline({"linter:E501": 4}), _baseline({}))
...
def test_a_signature_whose_check_did_not_run_is_not_fixed() -> None:
    ...
    narrow: an over-matching clear turns "the check stopped running" into
    "the problem went away", which is the mechanism deleting itself quietly.

    And the check going dark is a REGRESSION, not a silent note. The two
    ...
    check stopped, and the verdict follows the second.
    """
    ...
    comparison = baseline.compare(base, current)
...
def test_a_decreased_count_is_not_a_regression() -> None:
    comparison = baseline.compare(_baseline({"linter:E501": 9}), _baseline({"linter:E501": 2}))
...
def test_buckets_are_sorted_by_signature() -> None:
    ...
    comparison = baseline.compare(_baseline({}), current)
...
def test_exit_code_table(regressed: bool, fail_on_regression: bool, expected: int) -> None:
    ...
    assert baseline.exit_code_for(comparison, fail_on_regression=fail_on_regression) == expected


# --- the schema note ----------------------------------------------------


def test_a_check_schema_change_is_a_note_not_a_refusal() -> None:
    comparison = baseline.compare(
        _baseline({}, check_schema_version=2),
        _baseline({}, check_schema_version=3),
    )

    assert comparison.check_schema_changed == (2, 3)
    assert comparison.regressed is False


def test_an_unchanged_check_schema_records_no_note() -> None:
    assert baseline.compare(_baseline({}), _baseline({})).check_schema_changed is None
...
def _from(result: VerificationResult) -> baseline.Baseline:
    return baseline.baseline_from_result(
        result,
        base_ref="abc1234def",
        project=PROJECT,
        generated_at="2026-09-06T00:00:00Z",
...
def test_a_check_that_is_both_a_row_and_a_gap_counts_as_unmeasured() -> None:
    ...
    clearing side has to be the narrow one whatever the check emits."""
    result = _result(
        [_failing_linter()],
...
def test_passed_is_the_checks_own_verdict_not_an_empty_signature_map() -> None:
...
def _comparison(**kwargs: Any) -> baseline.Comparison:
    defaults: dict[str, Any] = {
        ...
        "unmeasured": {},
        "stopped_measuring": {},
        "check_schema_changed": None,
        "project_changed": None,
    }
    return baseline.Comparison(**{**defaults, **kwargs})


def test_human_report_heading_and_verdict() -> None:
    lines = baseline_report.render_human(_comparison(), _baseline({}), Path("scripts/b.json"))

    assert lines[0] == "check regression report vs scripts/b.json (0123456)"
...
def test_human_report_names_the_counts_when_it_regressed() -> None:
    ...
    lines = baseline_report.render_human(comparison, _baseline({}), Path("b.json"))
...
def test_human_report_says_unknown_when_the_baseline_has_no_commit() -> None:
    lines = baseline_report.render_human(
...
def test_markdown_first_line_is_the_marker_exactly() -> None:
    ...
    text = baseline_report.render_markdown(_comparison(), _baseline({}), Path("b.json"))

    marker = baseline_report.MARKDOWN_MARKER
...
def test_markdown_carries_a_table_per_non_empty_bucket() -> None:
    ...
    text = baseline_report.render_markdown(comparison, _baseline({}), Path("b.json"))
...
def test_the_footer_says_what_the_mode_in_force_does() -> None:
    """`docs/baseline.md` says adding --fail-on-regression is the whole of
    ...
    advisory = baseline_report.render_markdown(comparison, base, Path("b.json"))
    blocking = baseline_report.render_markdown(
        comparison, base, Path("b.json"), fail_on_regression=True
    )

    assert advisory.endswith(baseline_report.ADVISORY_FOOTER)
    assert blocking.endswith(baseline_report.BLOCKING_FOOTER)
    assert baseline_report.ADVISORY_FOOTER not in blocking
    assert baseline_report.BLOCKING_FOOTER not in advisory
...
def test_both_renderers_carry_the_schema_note() -> None:
    comparison = _comparison(check_schema_changed=(2, 3))
    base = _baseline({})

    human = "\n".join(baseline_report.render_human(comparison, base, Path("b.json")))
    markdown = baseline_report.render_markdown(comparison, base, Path("b.json"))
...
def test_the_write_summary_line_names_the_unmeasured_checks() -> None:
    ...
    quiet = baseline.write_summary_line(Path("b.json"), _baseline({"linter:E501": 12}))
    holed = baseline.write_summary_line(
        Path("b.json"),
...
def test_the_json_block_carries_every_bucket_and_the_verdict() -> None:
    comparison = _comparison(new={"a:b": 1}, increased={"c:d": (1, 2)}, check_schema_changed=(2, 3))

    document = baseline_report.comparison_document(
        comparison,
        _baseline({"c:d": 1}),
    ...
    assert document["check_schema_changed"] == {"baseline": 2, "current": 3}
    assert document["baseline"]["schema_version"] == 2
...
def _green_committed_baseline() -> baseline.Baseline:
    return _baseline(
        {},
...
def test_a_branch_whose_test_suite_stops_finishing_is_a_regression() -> None:
    ...
    comparison = baseline.compare(_green_committed_baseline(), current)
    ...
    assert comparison.regressed is True
    assert baseline.exit_code_for(comparison, fail_on_regression=True) == 1


def test_a_check_switched_off_entirely_is_also_a_check_that_stopped() -> None:
    """Set difference over ``measured_checks``, so it covers both ways a check
    goes dark: a row that measured nothing, and no row at all."""
    current = _baseline({}, measured=("linter",))

    comparison = baseline.compare(_baseline({}, measured=("linter", "typecheck")), current)

    assert comparison.stopped_measuring == {"typecheck": baseline.NO_REASON_RECORDED}
    assert comparison.regressed is True


def test_a_check_the_baseline_never_measured_does_not_flag_when_it_is_still_off() -> None:
...
def test_both_renderers_name_the_check_and_the_reason() -> None:
    ...
    markdown = baseline_report.render_markdown(comparison, _baseline({}), Path("b.json"))
...
def test_the_json_block_carries_the_stopped_checks() -> None:
...
def test_a_check_the_baseline_never_measured_can_never_be_a_stopped_check() -> None:
    """The structural rule `docs/baseline.md` got wrong (#400).
    ...
    `test_a_check_the_baseline_never_measured_does_not_flag_when_it_is_still_off`
    above.
    """
...
#: The sentence `docs/baseline.md` carried until #400, quoted so the fix cannot
...
def test_the_doc_no_longer_claims_those_holes_show_up_as_stopped_measuring() -> None:
    ...
    The positive half lives in `tests/test_check_baseline_cli.py`, which
...
def test_neither_the_doc_nor_the_docstring_claims_every_line_was_added() -> None:
    ...
    come back.
    """
    flat_doc = " ".join(BASELINE_DOC.read_text(encoding="utf-8").split()).lower()
    flat_docstring = " ".join((baseline.compare.__doc__ or "").split()).lower()
