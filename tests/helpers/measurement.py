...
say it measured nothing, and does the baseline then put a baseline signature in
...
def assert_unmeasured(row: CheckResult) -> None:
    """The row measured nothing, and the baseline treats it accordingly.
    ...
    # The flagging half of the same fact: the check went dark, so the verdict
