...
#: file's own base/feature commits.
...
def test_a_mutmut_run_that_exits_two_is_scored_not_failed(
    ...
    assert len([c for c in result.checks if c.name == "mutation_testing"]) == 1
    row = _row(result)
...
def test_the_run_names_its_own_tests_dir_and_not_the_projects(
    ...
    recdir = put_mutmut_on_path(tmp_path, monkeypatch, junit=junit((1, "mod.py", 6, "killed")))
    result = _run(tmp_path)
    assert [c for c in result.checks if c.name == "mutation_testing"] != []
...
def test_a_fatal_mutmut_exit_is_a_sidecar(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """D1: a fatal mutmut exit (bit 1 set) is ``command_failed``, and no
    report spawn follows a run that never produced one. Real failure
    measured without the ``whatthepatch`` extra (measurements 2a)."""
    _repo(tmp_path)
    ...
    result = _run(tmp_path)
    gap = only_gap(result, "mutation_testing", "command_failed")
...
def test_an_unparseable_report_is_a_sidecar_not_a_zero(
    ...
    """CLAUDE.md: what cannot be parsed is rejected, never counted as
    zero. The detail assertion is what makes this RED before the fix
    for its own reason rather than by coincidence: on this fixture
    (tests at the repo root) the pre-fix driver already gaps
    ``command_failed`` - it never passes ``--tests-dir`` at all
    (reproduction 2), so the fake refuses the run before reaching the
    report it was given. Only the detail - "the mutation report could
    not be read", which only the parse-failure path writes - separates
    that coincidence from the property this test names.
    """
    _repo(tmp_path)
...
def test_a_preexisting_backup_refuses_before_any_spawn(
    ...
    """D7 (shared with Layer 2): a pre-existing ``.bak`` beside a target
    is refused, never silently overwritten - mutmut writes ``<file>.bak``
    before it mutates, so kstrl cannot tell its backup from this one.

    RED before the fix: Layer 1 has no such refusal, so mutmut is
    spawned.
    """
    _repo(tmp_path)
...
def test_a_stale_cache_is_deleted_before_the_run(
    ...
    hard-codes the path, so there is no relocation knob)."""
    _repo(tmp_path)
...
def test_read_only_records_a_gap_and_writable_records_a_row(
    ...
    """mutmut rewrites the files it mutates, so it cannot run under
    ``ks sense``. Replaces
    ``tests/test_verify.py::TestReadOnlyVerification::test_read_only_is_the_only_reason_the_mutation_row_is_missing``,
    written against the fake binary instead of a stubbed ``run_scrubbed``
    - the row being absent for the wrong reason is exactly what that
    test's docstring said it was written to prevent."""
    _repo(tmp_path)
    recdir = put_mutmut_on_path(tmp_path, monkeypatch, junit=junit((1, "mod.py", 6, "killed")))
    result = _run(tmp_path, read_only=read_only)
    if read_only:
        only_gap(result, "mutation_testing", "read_only")
        assert not (recdir / "argv-run.txt").exists()
    else:
        assert [c for c in result.checks if c.name == "mutation_testing"] != []
        assert [g for g in result.not_measured if g.check == "mutation_testing"] == []
