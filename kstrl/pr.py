...
from kstrl.pr_state import _pr_mergeable, _pr_state
...
#: open-PR bound, the baseline, the polled steering channel (#231) -
...
class MergeConfirmation:
    """What one merge poll concluded, and the ref it observed.
...
def wait_for_merge(
    ...
    Returns a :class:`MergeConfirmation` whose ``state`` is "merged",
    "closed" (closed without merge), or "pending" (timeout: state
    unknown, re-pollable). Each poll is individually bounded so a hung
    gh cannot outlive the deadline by much.
    """
    ...
    return MergeConfirmation(state="pending")
...
def _merge_and_wait(
    ...
    confirmation = wait_for_merge(pr_number, cwd, timeout=merge_timeout)
...
def _generate_pr_body(
    ...
    #
    # R10.3 adds claim disagreements to the same callout set, for
    ...
    # does. A claim finding has a category that means something and
